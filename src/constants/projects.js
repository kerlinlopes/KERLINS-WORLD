export const PROJECTS = [
  {
    id: '1',
    title: 'Trivia Game',
    description:
      'A SCREAM themed trivia game containing 20 questions',
    image: {
      src: 'https://i.imgur.com/LkTnF0f.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://seir-penguin-project-1-theta.vercel.app/',
      repo: 'https://github.com/kerlinlopes/seir_penguin_project_1',
    },
    technologies: [
      '#HTML',
      '#CSS',
      '#javascript',
      '#jQuery',
      '#vercel',
    ],
  },
  {
    id: '2',
    title: 'Journals App',
    description:
      ' A journal app to put in your thoughts, to-list or whatever else. Has user authentication',
    image: {
      src: 'https://i.imgur.com/EKARX9p.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://kl-penguin-p2.herokuapp.com/',
      repo: 'https://github.com/kerlinlopes/seir_penguin_project_2',
    },
    technologies: [
      '#node',
      '#nodejs',
      '#express',
      '#mongodb',
      '#liquidjs',
      '#javascript',
      '#mongoose',
      '#CSS',
    ],
  },
  {
    id: '3',
    title: 'Grocery List App',
    description:
      'A group project that created a full CRUD app built with react and css. Lets a user add grocery items and or edit, update and delete them.',
    image: {
      src: 'https://i.imgur.com/jrL76gZ.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://xenodochial-euclid-2ab811.netlify.app/products',
      repo: 'https://github.com/gadgetgeek/kweb_frontend',
    },
    technologies: [
      '#HTML5',
      '#javascript',
      '#scss',
      '#css',
      '#reactjs',
      '#netlify',
      '#mongodb',
    ],
  },
  {
    id: '4',
    title: 'Plants Scrapbook ',
    description:
      'A Plants Scrapbook that allows a user to add a plant and or edit, update and delete. Also allows the user to see a zodiac page and see what flower their astrological sign is assigned to and a guide page with a carousel to learn how to care for your plants.',
    image: {
      src: 'https://i.imgur.com/8v2uy0y.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://plants-scrapbook-frontend-k59j9j2tz-kerlinlopes.vercel.app/',
      repo: 'https://github.com/kerlinlopes/plants-scrapbook-frontend',
    },
    technologies: [
      '#javascript',
      '#HTML',
      '#CSS',
      '#vercel',
      '#Python',
    ],
  },
  {
    id: '5',
    title: 'Secret Website',
    description:
      'An employee tracking secret app for a ceo. Has 3 pages that are an employee page, an about page and a TOP INFO page. The employee page is where I am tracking my employees information. The about page has my employees pictures and my companies values. The TOP INFO page has my notes and thoughts that I have, about my employees. My employees do not know I have this.. it is my secret website... mwahahaha',
    image: {
      src: 'https://i.imgur.com/51FDLOj.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://company-employees.vercel.app/',
      repo: 'https://github.com/kerlinlopes/company-employees',
    },
    technologies: [
      '#HTML5',
      '#javascript',
      '#scss',
      '#css',
      '#react',
      '#vercel',
      '#mongodb',
      '#NextJS',
      '#CRUDapplication'
    ],
  },
  {
    id: '6',
    title: 'Memories App',
    description:
      'A social media app where users can post any events happening in their lives. It includes a form where you can add the creator of the post, a title, a message, hashtags and include a photo as well! User is able to create a post, update it, read it and or delete it. ',
    image: {
      src: 'https://i.imgur.com/ai2GJTg.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://memories-app-kl.netlify.app/',
      repo: 'https://github.com/kerlinlopes/memories-project-kl',
    },
    technologies: [
      '#HTML5',
      '#javascript',
      '#css',
      '#react',
      '#netlify',
      '#mongodb',
      '#express',
    ],
  },
  {
    id: '7',
    title: 'Nail Business Page',
    description:
      'A nail business portfolio. It contains a logo, nav bar, about section, service section, contact section and footer section. This project was made just to show my skills in HTML and CSS. Very easy and simple web design page. ',
    image: {
      src: 'https://i.imgur.com/hgzWi0N.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://krazy-k-nails.netlify.app/',
      repo: 'https://github.com/kerlinlopes/Nail-Tech-Mock-Portfolio',
    },
    technologies: [
      '#HTML5',
      '#css',
      '#netlify',
    ],
  },
  {
    id: '8',
    title: 'Royal Reyes Landscaping',
    description:
      'A landscaping business page. It has a home, pricing and contact page. It is suppose to "mock" how I would most likely develop a business page.',
    image: {
      src: 'https://i.imgur.com/nnRgmtz.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://royalreyeslandscaping-mock.vercel.app/',
      repo: 'https://github.com/kerlinlopes/RoyalReyesLandscaping-mock',
    },
    technologies: [
      '#HTML5',
      '#css',
      '#vercel',
      '#react',
      '#javascript',
      'framer-motion',
      '#svg',
    ],
  },
  {
    id: '8',
    title: 'Museum Of Candy',
    description:
      'A museum of candy website using bootstrap.',
    image: {
      src: 'https://i.imgur.com/Lc4IreG.png',
      placeholderSrc:
        'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAzIDIiPjwvc3ZnPg==',
    },
    links: {
      site: 'https://bootstrap-candy.netlify.app/',
      repo: 'https://github.com/kerlinlopes/museum-of-candy-bootstrap',
    },
    technologies: [
      '#HTML5',
      '#css',
      '#netlify',
      '#javascript',
      '#udemy',
    ],
  },
];
